...
* `tags_regex` - A regular expression used to trigger a Workspace run for matching Git tags. Cannot be used if `trigger-patterns`, `trigger-prefixes` or `file_triggers_enabled` has been set.
