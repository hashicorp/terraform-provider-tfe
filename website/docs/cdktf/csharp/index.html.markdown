...
- `Hostname` (String) The Terraform Enterprise hostname to connect to. Defaults to `app.terraform.io`. Can be overridden by setting the `TfeHostname` environment variable.
- `Organization` (String) The default organization that resources should belong to. If provided, it's usually possible to omit resource-specific `Organization` arguments. Ensure that the organization already exists prior to using this argument. This can also be specified using the `TfeOrganization` environment variable.
- `SslSkipVerify` (Boolean) Whether or not to skip certificate verifications. Defaults to `False`. Can be overridden setting the `TfeSslSkipVerify` environment variable.
