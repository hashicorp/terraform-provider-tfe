...
* `speculative_plan_management_enabled` - (Optional) Whether or not to enable Speculative Plan Management. If true, pending VCS-triggered speculative plans from older commits will be cancelled if a newer commit is pushed to the same branch.
