...
- `tag_match_logic` (Optional) Controls how this policy set matches workspaces by tags. `"any"` (default) — applies to workspaces that have at least one of the configured tags. `"all"` — applies only to workspaces that have every configured tag. Applies to both tag inclusions (`tfe_tag_policy_set`) and tag exclusions (`tfe_tag_policy_set_exclusion`). Note: a second `terraform apply` may be required after adding the first tag before this value is visible.
