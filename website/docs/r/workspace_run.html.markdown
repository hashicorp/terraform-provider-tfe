...
* `allow_config_version_missing` - (Optional) Only supported in the `destroy` block. Whether or not to treat a missing configuration version as a success rather than an error when creating the destroy run. This is useful for `destroy` runs against workspaces that never had a configuration version uploaded (for example, an empty workspace). When set to `true` and the run cannot be created because the configuration version is missing, the destroy is treated as a no-op success. Setting this in the `apply` block is not allowed and will produce an error. Defaults to `false`.
