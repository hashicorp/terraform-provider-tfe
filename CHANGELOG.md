...
* `r/tfe_workspace_run`: Destroy runs against workspaces with no configuration version (e.g. empty workspaces that never had a configuration uploaded) are now treated as a no-op success instead of returning an error. ([#1939](https://github.com/hashicorp/terraform-provider-tfe/issues/1939))
