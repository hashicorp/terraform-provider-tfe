...
func getClient(tfeHost, token string, insecure bool) (*tfe.Client, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("Required token could not be found. Please set the token in the provider configuration, or using the TFE_TOKEN environment variable.")
	}
