...
func dataSourceTFERemoteStateRead(d *schema.ResourceData, meta interface{}) error {
	...
	wsID := d.Get("workspace").(string)
	log.Printf("[DEBUG] Read remote state for and Workspace %s", wsID)

	sv, err := tfeClient.StateVersions.Current(ctx, wsID)
	if err != nil {
		if err == tfe.ErrResourceNotFound {
			return fmt.Errorf("Could not read  remote state for workspace '%s'", wsID)
		}
