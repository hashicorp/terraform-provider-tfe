...
func resourceTFERegistryModuleCreateWithoutVCS(meta interface{}, d *schema.ResourceData) (*tfe.RegistryModule, error) {
	...
	if v, ok := d.GetOk("no_code"); ok {
		...
		log.Println("[WARN] no_code is deprecated as of release 0.33.0 and may be removed in a future version. The preferred way to create a no-code registry module is to create a tfe_no_code_module resource for the tfe_registry_module resource.")
