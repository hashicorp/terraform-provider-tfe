...
func TestResourceTFENotificationConfigurationDeleteIgnoresNotFound(t *testing.T) {
	...
	r := &resourceTFENotificationConfiguration{config: ConfiguredClient{ClientV2: client}}
	ctx := context.Background()
	schemaResp := &fwresource.SchemaResponse{}
	r.Schema(ctx, fwresource.SchemaRequest{}, schemaResp)
	state := tfsdk.State{Schema: schemaResp.Schema}
	diags := state.Set(ctx, &modelTFENotificationConfiguration{
