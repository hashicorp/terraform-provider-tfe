...
func (e *TeamTokenEphemeralResource) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	var data TeamTokenEphemeralResourceModel
	...
	var team_id string
	// resp.Diagnostics.Append(e.config.dataOrDefaultId(ctx, req.Config, &team_id)...)
	// if resp.Diagnostics.HasError() {
	// 	return
	// }
	...
	result, err := e.config.Client.TeamTokens.CreateWithOptions(ctx, team_id, options)
	if err != nil {
		resp.Diagnostics.AddError("Unable to read resource", err.Error())
		return
	}

	data = ephemeralResourceModelFromTFETeamToken(team_id, result)
...
// tfe.TeamToken value.
func ephemeralResourceModelFromTFETeamToken(team string, v *tfe.TeamToken) TeamTokenEphemeralResourceModel {
	return TeamTokenEphemeralResourceModel{
		TeamId:    types.StringValue(v.ID),
