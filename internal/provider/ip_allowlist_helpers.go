...
// IP allowlists are referred to as "CIDR range lists" in the HCP Terraform API and go-tfe v2 client.
// The enforcement_scope values below map to the API enum.
const (
...
// modelTFECIDRRange is the model for a single CIDR range nested within an IP allowlist.
// The range identifier assigned by the API is intentionally not exposed; ranges are
// reconciled by their CIDR value.
...
// cidrRangeAttrTypes describes the attribute types of a single cidr_range element.
// It is used when converting API results back into a types.Set.
...
// isV2ResourceNotFound reports whether the given go-tfe v2 client error represents an HTTP 404 Not Found response.
...
// enforcementScopeToV2 converts the Terraform enforcement_scope string into the go-tfe v2 enum value.
...
// enforcementScopeFromV2 converts the go-tfe v2 enum value into the Terraform enforcement_scope string.
...
// readIPAllowlistRanges fetches every CIDR range that belongs to a CIDR range list, transparently following pagination.
...
// cidrRangeSetFromAPI converts API CIDR ranges into a types.Set of cidr_range objects for storage in Terraform state.
...
// findIPAllowlistByName searches an organization's CIDR range lists for one with the given name,
// following pagination. It returns the matching list's ID and whether a match was found.
