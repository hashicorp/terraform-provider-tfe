	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"organization": {
				...
				Computed:    true,
				ForceNew:    true,
				Description: "The name of the organization associated with the registry module. Must be set if `module_provider` is used, or if `vcs_repo` is used via a GitHub App. If omitted, organization must be defined in the provider config.",
			},
			"module_provider": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Specifies the Terraform provider that this module is used for. For example, `aws`. Required with `name`.",
			},
			"name": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				ForceNew:    true,
				Description: "The name of the registry module. Must be set if `module_provider` is used.",
			},
			"publishing_mechanism": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The publishing mechanism used when releasing new versions of the module.",
			},
			"vcs_repo": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"tags": {
							...
							Optional:    true,
							Computed:    true,
							Description: "Whether tag-based publishing is enabled for the registry module. When `true`, `branch` must be set to an empty value.",
						},
						"source_directory": {
							Type:        schema.TypeString,
							Optional:    true,
			...
			"namespace": {
				...
				Description:  "The namespace of a public registry module. Can be used if `module_provider` is set and `registry_name` is `public`.",
			},
			"no_code": {
				Type:        schema.TypeBool,
				Optional:    true,
			...
			"registry_name": {
				...
				Description: "Whether the registry module is `private` or `public`. Can be used if `module_provider` is set.",
			},
			"test_config": {
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Description: "Settings for running tests for the registry module.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"tests_enabled": {
							Type:        schema.TypeBool,
							Optional:    true,
							Description: "Whether tests are enabled for the registry module. Tests are only supported for branch-based publishing.",
						},
						...
						"agent_pool_id": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "The ID of an agent pool to use for registry module tests. Requires `agent_execution_mode` to be `agent`. Beta feature, not available to all users.",
						},
					},
				},
			},
			"initial_version": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "The initial version for a branch-based module. Can be used if `vcs_repo.branch` is set. Defaults to `0.0.0`.",
			},
		},
	}
}
