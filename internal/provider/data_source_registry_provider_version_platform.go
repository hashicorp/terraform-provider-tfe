...
var (
	_ datasource.DataSource              = &dataSourceTFERegistryProviderVersionPlatform{}
	_ datasource.DataSourceWithConfigure = &dataSourceTFERegistryProviderVersionPlatform{}
)

// NewRegistryProviderVersionPlatformDataSource is a helper function to simplify the provider implementation.
func NewRegistryProviderVersionPlatformDataSource() datasource.DataSource {
	return &dataSourceTFERegistryProviderVersionPlatform{}
}

// dataSourceTFERegistryProviderVersionPlatform is the data source implementation.
type dataSourceTFERegistryProviderVersionPlatform struct {
	config ConfiguredClient
}

// Metadata returns the data source type name.
func (d *dataSourceTFERegistryProviderVersionPlatform) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_registry_provider_version_platform"
}

// Schema defines the schema for the data source.
func (d *dataSourceTFERegistryProviderVersionPlatform) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "This data source can be used to retrieve a provider version platform from the private registry.",
...
func (d *dataSourceTFERegistryProviderVersionPlatform) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *dataSourceTFERegistryProviderVersionPlatform) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data modelTFERegistryProviderVersionPlatform
	...
	osArch := data.OSArch.ValueString()
	...
	platform, err := d.config.Client.RegistryProviderPlatforms.Read(ctx, platformID)
	if err != nil {
		resp.Diagnostics.AddError("Unable to read registry provider version platform", err.Error())
		return
	}

	result := modelFromTFERegistryProviderVersionPlatform(platform, organization, registryName, namespace, data.ProviderName.ValueString(), data.Version.ValueString())
